import { pgSchema, pgEnum, pgTable, uuid, text, varchar, integer, bigint, bigserial, timestamp, jsonb, customType, json, boolean, inet, smallint, index, uniqueIndex, foreignKey, primaryKey, unique, check, doublePrecision, numeric } from "drizzle-orm/pg-core"
...
export const imageStatus = pgEnum("ImageStatus", ["PENDING", "PROCESSING", "FAILED", "COMPLETED"])
...
export const photos = pgTable.withRLS("photos", {
	id: uuid().defaultRandom().primaryKey(),
	originalName: varchar({ length: 200 }).notNull(),
